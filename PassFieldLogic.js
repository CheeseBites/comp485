  ...
  if (mouseupindicator == 0) {
    if (password.indexOf(id) == -1) {
      ...
      document.getElementById(
        id
      ).style.backgroundImage = selectedBulletImageURL;
...
function drawLowSloppedPositiveLine(
  xCoordinatePassed,
  yCoordinatePassed,
  prevXPassed,
  prevYPassed
...
function drawLowSloppedNegativeLine(
  xCoordinatePassed,
  yCoordinatePassed,
  prevXPassed,
  prevYPassed
...
function drawGeneralLowSloppedLine(
  previousYPassed,
  yCoordinatePassed,
  previousXPassed,
  newElementClassName
...
function drawHighSloppedNegativeLine(
  xCoordinatePassed,
  yCoordinatePassed,
  prevXPassed,
  prevYPassed
...
function drawHighSloppedPositiveLine(
  xCoordinatePassed,
  yCoordinatePassed,
  prevXPassed,
  prevYPassed
...
function drawGeneralHighSloppedLine(
  previousXPassed,
  xCoordinatePassed,
  previousYPassed,
  newElementClassName
  ...
  iDiv.style.top = 43;
  document.getElementById("mainElement").appendChild(iDiv);
  var divTopValueOneForHighSloppedLines =
    109 +
    110 * (previousYPassed - 1 >= 0 ? previousYPassed - 1 : previousYPassed);
  var divTopValueTwoForHighSloppedLines =
    109 + 110 * (previousYPassed >= 0 ? previousYPassed : 0);
  if (previousXPassed > xCoordinatePassed) {
    iDiv.style.left =
      newElementClassName == "highSlopePositive"
        ? divTopValueTwoForHighSloppedLines
        : divTopValueOneForHighSloppedLines;
  } else {
    iDiv.style.left =
...
function drawGeneralStraightLine(
  prevXYPassedtodraw,
  xycoordinatetodraw,
  lastSpecificParameter,
  newElementClassName
) {
  var iDiv = document.createElement("div");
  iDiv.className = newElementClassName;
  ...
  if (prevXYPassedtodraw < xycoordinatetodraw) {
    ...
    if (Math.abs(prevXYPassedtodraw - xycoordinatetodraw) > 1) {
      if (isVerticalLine) {
        iDiv.style[topStyle] =
          54 + isVerticalLine + 110 * (prevXYPassedtodraw - 2);
      } else {
        iDiv.style[leftStyle] =
          54 + isVerticalLine + 110 * (prevXYPassedtodraw - 2);
      }
    } else {
      if (isVerticalLine) {
        iDiv.style[topStyle] =
          54 + isVerticalLine + 110 * (prevXYPassedtodraw - 1);
      } else {
        iDiv.style[leftStyle] =
          54 + isVerticalLine + 110 * (prevXYPassedtodraw - 1);
      }
  ...
  if (Math.abs(prevXYPassedtodraw - xycoordinatetodraw) > 1) {
    if (isVerticalLine) {
      iDiv.style[heightStyle] +=
        Math.abs(prevXYPassedtodraw - xycoordinatetodraw) * 110;
    } else {
      iDiv.style[widthStyle] +=
        Math.abs(prevXYPassedtodraw - xycoordinatetodraw) * 110;
    }
  }
  if (isVerticalLine) {
...
function drawNegativeDiagonalLine(
  xcoordinatetodraw,
  ycoordinatetodraw,
  prevXPassedtodraw,
  prevYPassedtodraw
...
function createGeneralDiagonalLine(
  xcoordinatetodraw,
  ycoordinatetodraw,
  prevXPassedtodraw,
  prevYPassedtodraw,
  ...
  if (ycoordinatetodraw > prevYPassedtodraw) {
    ...
    iDiv.style.top =
      32 +
      (isPositiveDiagonalLine ? prevXPassedtodraw - 1 : prevXPassedtodraw) *
        110;
  } else {
    iDiv.style.left = 110 + (prevYPassedtodraw - 1) * 110;
    iDiv.style.top =
      32 +
      (isPositiveDiagonalLine ? prevXPassedtodraw : prevXPassedtodraw - 1) *
        110;
  }
...
function drawLine(
  ...
  if (
    prevXPassed - xCoordinatePassed ==
    horizontalVerticalLinesDifferenceParameter
  ) {
    drawHorizontalLine(
      xCoordinatePassed,
      yCoordinatePassed,
  ...
  //Definitely Vertical line as y coordinate difference is zero
    ...
    (prevYPassed - yCoordinatePassed) / (prevXPassed - xCoordinatePassed) ==
    -lowSloppedLinesDifferenceParameter
  ) {
    drawLowSloppedPositiveLine(
      xCoordinatePassed,
      yCoordinatePassed,
      prevXPassed,
      prevYPassed
    ...
    (prevYPassed - yCoordinatePassed) / (prevXPassed - xCoordinatePassed) ==
    highSloppedLinesDifferenceParameter
  ) {
    drawHighSloppedNegativeLine(
      xCoordinatePassed,
      yCoordinatePassed,
      prevXPassed,
      prevYPassed
  ...
  //We have to draw a diagonal line now
    ...
    var angle = convertAngleFromRadiansToDegrees(
      Math.atan2(
        prevYPassed - yCoordinatePassed,
        prevXPassed - xCoordinatePassed
      )
    ...
    if (
      angle == -diagonalLineSmallerAngle ||
      angle == diagonalLineBiggerAngle
    ) {
